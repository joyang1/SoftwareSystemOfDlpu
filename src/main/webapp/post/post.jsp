...
<div class="container">
    ...
    <table class="table table-striped">
        <tr style="color:#000; font-size: 16px">
            <td width="600px">帖子标题</td>
            <td width="350px">发帖人</td>
            <td width="350px">时间</td>
        </tr>
        <% if (startpost >= 0) {%>
        <%for (int i = startpost; i >= stoppost; i--) {%>
        <tr>
            <td width="500px"><a href="PostReplyServlet?id=<%=i%>" class="a" target="_blank"><%=name[i]%>
            </a></td>

            <td width="400px"><%=owner[i]%>
            </td>
            <td><%=time[i]%>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><%if (pageNow < pageCount) {%>
                第<span style="color:#111111; font-size: 16px"><i><%=pageNow%></i></span>页 共<span
                        style="color: #111111; font-size: 16px"><i><%=pageCount%></i></span>页
                <%} else {%>
    ...
    <form action="FSPostServlet" method="POST">
        ...
        <textarea type="text" rows="8" name="p_content"
                  style="width: 1165px;height: 200px; visibility:hidden;"><%=htmlspecialchars(htmlData)%></textarea>
